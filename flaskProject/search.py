...
class ForText():
    ...
    def search_lemmatized_content(self):
        morph = pymorphy2.MorphAnalyzer()
        # Лемматизация поискового запроса
        lemmatized_query = ' '.join([morph.parse(word)[0].normal_form for word in query.split()])

        # Выполнение поиска по лемматизированному тексту
        self.cur.execute("SELECT lexeme, pos FROM dictionary WHERE lemmatized_text LIKE ?", ('%' + lemmatized_query + '%',))
